...
def load_data_singleview(args, path, modality: str, node_labels, dataset_name=None):
...
def build_dataset(a1, y, node_features=None, dataset_name=None, modality=None, args=None):
    x1 = compute_x(a1, args=args) if args is not None else compute_x(a1, node_features, dataset_name, modality)
...
def compute_x(a1, node_features=None, dataset_name=None, modality=None, args=None):
    # construct node features X
    nd_feat = node_features if node_features is not None else args.node_features
    if nd_feat == 'identity':
        x = torch.cat([torch.diag(torch.ones(a1.shape[1]))] * a1.shape[0]).reshape([a1.shape[0], a1.shape[1], -1])
        x1 = x.clone()

    elif nd_feat == 'node2vec':
        X = np.load(f'./{args.dataset_name}_{args.modality}.emb', allow_pickle=True).astype(np.float32) if args is not None \
            else np.load(f'./{dataset_name}_{modality}.emb', allow_pickle=True).astype(np.float32)
    ...
    elif nd_feat == 'degree_bin':
        a1b = (a1 != 0).float()
        x1 = binning(a1b.sum(dim=2))

    elif nd_feat == 'adj': # edge profile
        x1 = a1.float()

    elif nd_feat == 'LDP': # degree profile
        a1b = (a1 != 0).float()
        x1 = []
