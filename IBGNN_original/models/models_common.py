...
import torch
import sys
sys.path.append('/home/neuro-gnn-project/IBGNN/')
...
class IBGConv(torch.nn.Module):
    def __init__(self, input_dim, num_classes, args=None, hidden_dim=None, n_GNN_layers=None, pooling=None):
        super(IBGConv, self).__init__()
        self.activation = torch.nn.ReLU()
        self.convs = torch.nn.ModuleList()

        hidden_dim = args.hidden_dim if args is not None else hidden_dim
        num_layers = args.n_GNN_layers if args is not None else n_GNN_layers
        self.pooling = args.pooling if args is not None else pooling
